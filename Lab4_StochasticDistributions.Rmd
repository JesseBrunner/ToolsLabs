...
The figure below shows for an arbitrary distribution the R commands showing the density function (`ddist()`), cumulative distribution function (`pdist()`), quantile function (`qdist()`), and random‐deviate function (`rdist()`) where "`dist`" is the name of the distributions (e.g., `binom`, `norm`, `pois`, `chi`, etc.). 
...
```{r}
# This is the probability of exactly 2 successes out of 15 trials, with a 15% chance of sucess on each trial 
...
x <- 0:15
qplot(x, y = dbinom(x, size = 15, prob = 0.15), xlab = "number of successes", ylab = "probability") # default geom = "point"
# Or alternatively
qplot(x, ymax=dbinom(x, size = 15, prob = 0.15), ymin = 0, geom="linerange", , xlab = "number of successes", ylab = "probability")
```
...
A plot can illsutrate what cummualative probability distribution looks like.
...
Finally, `qbinom(p, size, prob)` gives the quantile function, where `p` is the proportion of the area under the pdf, or identically, the value of the cdf you want to evaluate. The quantile function returns the value of $x$ associated with that cumulative probability, i.e., the value of $x$ such that $P(x ≤ p) = q$. (Note that the quantile function $Q$ is the inverse of the cumulative distribution function $C$: if $Q(p) = q$ then $C(q) = p$.)
