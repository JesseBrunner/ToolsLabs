...
A key aspect of any data analysis is to _know your data_. We have have learned a bit about getting summaries (e.g., with `summary()`, `mean()`, `sd()`, etc.), but plotting data is, argueably, far more important. Graphing can help you:  

  *  Find outlying observations, instrumental measurements, or just plain data entry errors 
  *  Identify important relationships between variables, see trends in distrubitons, etc.
...
```
or use the menus (`Tools\Install Packages...`) to do the same thing. (Pointing and clicking is OK in this case since intalling packages is not generally part of an analysis.)  If this is your first time using R/R Studio, you will probably be asked some questions about which repository (choose something close) and maybe where to save things. 
...
Let me try to explain how this works. First, `ggplot()` creates a coordinate system with the data you provide. You tell it which vaiables you want mapped to which aesthetics in the `aes()` call. In this case we want `Initial` mapped to the x-axis and `Killed` mapped to the y. This might seem odd or a bit overkill, but it will make sense as you begin to map more variables onto other aesthetics (e.g., color, shape or linetype, size, transparency). With the coordinate system set up you then add layers or geometries, which in this case was a point. We'll see lots more. 
...
But to illustrate, let's add a straignt up linear regrssion line in red:
...
And lastly, we might want to save our pretty work using the `ggsave()` function. This function is clever about using the extension of the filename to figure out the right format.
```{r, eval=FALSE}
...
If your enter this command several times you will get several different results because it ias _adding random noise_. If there is too much noise, and I think there is, we can control this a bit by specifying the height and width of the jittering.
```{r sizepred_jitters}
...
So now we have built up the full figure that Bolker presented. I hope are beginning to see the genious of building up plots layer by layer. It really is powerful! 
...
I like option 1 better, but it really depends on what you want to show. Anyway, now let's add in facetying by the other treatments.
```{r Predationplot2}
...
See how that works? Well, the best way to figure out these functins is to try a bunch of things. Try switching which factors you map to color, linetype, facets, etc. Try different geometries as well (e.g., point) with and without smoothing. Try changing the faceting command from ` ~ pred` to `pred ~ `. Try adding a nicer axis label with `+ labs()`. 
...
Notice that the lines showing the means curve. This is because this transformation is done _after_ everying is calculated. If we want the transformation beforehand, we would use `scale_x_XXX()`. 
...
Homework
-----------------
Using a data set of your own (or perhaps your advisor's, or if you cannot find one, ask me) create one publication-quality plot showing the most important feature(s) of your data. Include a 1-2 sentence caption as you would in a manuscript. Please post the html of your Rmd file to Piazza under the lab1 folder. 
