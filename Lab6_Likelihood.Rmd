...
```
So, more precisely, our 95% CI on $a$ is 0.4025 -- 0.6825! Can you repeat this for the other parameter, $h$? It would be a good exercise for you to give it a try, you could even write your own function to do the tedious bits, but if you a) already get it or b) feel like your sinking and just want to see how you might do this in "real life", see how to do all of this using the `bbmle` package, which I explain at the bottom of the end of this document.
...
```
This produces a matrix of NLL values, but for plotting we need a "long" data set with one combination of $a$, $h$, and NLL per row. So we convert it, and then label the column names.
```{r reshape}
library(reshape)
NLL.m <- melt(NLL)
head(NLL.m)
