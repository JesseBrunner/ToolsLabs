...
ggplot(data.frame(x=c(0,100)), aes(x=x)) + 
	stat_function(fun=dbetabinom, geom="bar",
								args=list(prob=0.2805014, size=100, theta=coef(holling2.beta.fit)["theta"])) +
	stat_function(fun=dbinom, args=list(prob=0.2805014, size=100), color="red") + 
...
tab <- df1 %>% group_by(Ticks) %>% 
	...
	mutate(Expected=100*dpois(x=Ticks,lambda=coef(pois1)["lambda"])) #Note we must multiply by 100 to get counts
tab
...
```{r}
ggplot(df2, aes(x=Ticks)) + stat_count()
```
