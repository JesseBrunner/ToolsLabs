...
To simulate data, we first need to define our parameters:
```{r nl.setpars}
a <- 20
b <- 1
...
```
With the "True" functions plotted against the data, the two groups seem quite distinct. But what if we just had the data and didn't know what truth was? Could we recover the "True" parameters? Would we find strong evidence that there were two unique groups?
...
The first thing we need to do is create a function that calculates the negative log-likelihood (NLL) for our hyperbolic model with negative binomial distributed data. This is analogous to a function that would calculate the sums of squares in a regression model. We will then use this function to see how well the model fits our simulated data, given a set of parameters.
...
m1
```
Well, we have a difference in NLLs of `r m1$value - m0$value`, but is that a lot? 
